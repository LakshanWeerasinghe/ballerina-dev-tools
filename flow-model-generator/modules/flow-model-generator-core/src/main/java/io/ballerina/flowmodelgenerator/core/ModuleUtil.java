...
public class ModuleUtil {
    ...
    static BuildProject getSampleProject() {
        ...
        Path projectPath = Paths.get(
                Objects.requireNonNull(ModuleUtil.class.getClassLoader().getResource(PROJECT_NAME)).getFile());
        projectPath = Path.of(Objects.requireNonNull(ModuleUtil.class.getClassLoader()
                        .getResource(PROJECT_NAME)).getFile().split("build")[0])
                .resolve("src").resolve("main").resolve("resources").resolve("sample");
        projectPath = Path.of(projectPath.toString().split(":")[1]);
